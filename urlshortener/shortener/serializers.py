...
def handle_url(original_url, custom_short_id=None, user=None, instance=None):
    ...
    if not normalized_url:
        return {'error': 'Invalid URL', 'status': status.HTTP_400_BAD_REQUEST}

    if custom_short_id:
        if ShortenedURL.objects.filter(short_id=custom_short_id).exists() and (instance is None or instance.short_id != custom_short_id):
            return {'error': 'Custom short ID is already in use', 'status': status.HTTP_409_CONFLICT}
    ...
    return {'data': URLInputSerializer(short_url).data, 'status': status.HTTP_200_OK}
...
class URLInputSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            'short_id': {'read_only': True}  # short_id только для чтения, его нельзя задать через API
        }
